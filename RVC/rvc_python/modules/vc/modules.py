...
class VC:
    ...
    def get_vc(self,sid,version = "v2", *to_return_protect):
        ...
        if "config" in self.cpt:
            ...
            if "weight" not in self.cpt and "model" in self.cpt:
                self.cpt["weight"] = self.cpt["model"]
            if "emb_g.weight" in self.cpt["weight"]:
                self.cpt["config"][15] = self.cpt["weight"]["emb_g.weight"].shape[0]  # spk_embed_dim
        ...
        print("Config:", self.cpt["config"])
        self.net_g = synthesizer_class.get((self.version, self.if_f0), SynthesizerTrnMs256NSFsid)(
            spec_channels=self.cpt["config"][0],
            segment_size=self.cpt["config"][1],
            inter_channels=self.cpt["config"][2],
            hidden_channels=self.cpt["config"][3],
            filter_channels=self.cpt["config"][4],
            n_heads=self.cpt["config"][5],
            n_layers=self.cpt["config"][6],
            kernel_size=self.cpt["config"][7],
            p_dropout=self.cpt["config"][8],
            resblock=self.cpt["config"][9],
            resblock_kernel_sizes=self.cpt["config"][10],
            resblock_dilation_sizes=self.cpt["config"][11],
            upsample_rates=self.cpt["config"][12],
            upsample_initial_channel=self.cpt["config"][13],
            upsample_kernel_sizes=self.cpt["config"][14],
            spk_embed_dim=self.cpt["config"][15],
            gin_channels=self.cpt["config"][16],
            sr=self.cpt["config"][17],
